...
<style>
	...
	<c:if test="${empty sessionScope.uid }"> <!-- session 에 uid 가 없거나 아무내용이 없으면 -->
		<a href="../login/login.do">login</a>
		
	</c:if>
	<c:if test="${not empty sessionScope.uid }">	<!-- session 에 uid 가 있으면 -->
	...
	<table width="500" border="1">
		...
		<c:forEach items="${list }" var = "n2">
			...
			<td><%=n.getHit() %></td>  --%>
				...
				<td>${n2.seq }</td>
				<!-- notcieDetail.jsp?c= 에서 c가 getParameter 로 받아야 할 이름 -->
				<td><a href="noticeDetail.do?seq=${n2.seq }&h=${n2.hit }">${n2.title }</a></td>
				<td>${n2.writer }</td>
				<td>${n2.regdate }</td>
				<td>${n2.hit }</td>
			</tr>
